 ...
 * The JWT token is read from localStorage (key: audiomatch_session) and
 * attached automatically to every authenticated request.
 *
 * LOCAL STORAGE layout (no database):
 *   audiomatch_session        – { token, id, email, username }   (current user)
 *   audiomatch_history_<email> – [ ...identificationResult ]     (per-user history)
 */
...
function getToken() {
  try {
    const session = JSON.parse(localStorage.getItem('audiomatch_session') || 'null');
...
export async function register({ name, email, password }) {
  ...
  localStorage.setItem('audiomatch_session', JSON.stringify(session));
  return session;
}
...
export function saveToHistory(userEmail, result) {
  const key = `audiomatch_history_${userEmail}`;
...
export function clearHistory(userEmail) {
  localStorage.removeItem(`audiomatch_history_${userEmail}`);
}
